...
## Task Co#### Project-Specific Guidelines
...
- [ ] **Git Merge Conflicts**: Always resolve merge conflicts before deployment
- [ ] **Duplicate Routes**: Prevent duplicate Flask route definitions
- [ ] **Production Compatibility**: Ensure code works in production environmentorkflow
...
- [ ] **Resolve Merge Conflicts**: Check for and resolve any Git merge conflicts
- [ ] **Validate Code Syntax**: Ensure no syntax errors from unresolved conflicts
...
#### 4. **Testing Phase**
- [ ] **Pre-Deployment Validation**: Check for merge conflicts and syntax errors
...
- [ ] **CI/CD Compatibility**: Ensure code deploys without errors
...
### CI/CD Deployment Prevention Guidelines

#### **🚨 Critical Pre-Deployment Checks**

Before pushing code to production, ALWAYS perform these checks:

#### **1. Git Merge Conflict Resolution**
- [ ] **Check for Conflict Markers**: Search for `<<<<<<<`, `=======`, `>>>>>>>` in all files
- [ ] **Resolve All Conflicts**: Ensure all merge conflicts are properly resolved
- [ ] **Test After Resolution**: Run application locally after resolving conflicts
- [ ] **Validate Syntax**: Ensure no syntax errors from unresolved conflicts

#### **2. Duplicate Route Prevention**
- [ ] **Check Flask Routes**: Ensure no duplicate `@app.route()` or `@blueprint.route()` definitions
- [ ] **Verify Function Names**: Ensure no duplicate function names in same blueprint
- [ ] **Test Route Registration**: Verify Flask application starts without route conflicts
- [ ] **Check Method Conflicts**: Ensure same route doesn't have conflicting HTTP methods

#### **3. Import and Dependency Validation**
- [ ] **Missing Imports**: Check for missing import statements
- [ ] **Circular Imports**: Verify no circular import dependencies
- [ ] **Package Dependencies**: Ensure all required packages are in requirements.txt
- [ ] **Python Path Issues**: Verify all modules can be imported correctly

#### **4. Pre-Production Testing**
- [ ] **Local Testing**: Run application locally before pushing
- [ ] **Docker Testing**: Test in Docker environment if using containers
- [ ] **Database Migration**: Verify database migrations work correctly
- [ ] **Environment Variables**: Check all required environment variables are set

#### **5. Code Quality Checks**
- [ ] **Syntax Validation**: Run Python syntax check (`python -m py_compile`)
- [ ] **Linting**: Run linting tools to catch potential issues
- [ ] **Static Analysis**: Use tools like flake8 or pylint
- [ ] **Security Scanning**: Run security analysis tools

#### **🔧 Common CI/CD Breaking Issues**

**Most Common Issues That Break Production:**
1. **Unresolved Git Merge Conflicts** - Leaves invalid syntax in files
2. **Duplicate Route Definitions** - Causes Flask registration errors
3. **Missing Import Statements** - Breaks application startup
4. **Circular Import Dependencies** - Causes import errors
5. **Missing Environment Variables** - Breaks configuration
6. **Database Schema Issues** - Causes database connection errors
7. **Missing Dependencies** - Breaks package imports
...
python -m py_compile blueprint/*.py
...
When CI/CD issues occur, document them using this template:
...
## Recent CI/CD Issue Resolution

### 🚨 ISSUE RESOLVED - July 5, 2025
**Problem**: Duplicate route definitions breaking EC2 deployment
**Root Cause**: Unresolved Git merge conflicts in `browse.py`
**Impact**: CI/CD pipeline failing, production deployment blocked

### **Specific Issue Details:**
- **File**: `blueprint/browse.py`
- **Problem**: Two `@browse_bp.route('/browse')` function definitions with same name `browseEscort()`
- **Cause**: Git merge conflict markers (`<<<<<<<`, `=======`, `>>>>>>>`) not properly resolved
- **Symptoms**: 
  - Flask route registration errors
  - Application startup failures
  - "Production breaking" deployment issues

### **Resolution Applied:**
1. **Removed duplicate route definition** ✅
2. **Resolved all Git merge conflicts** ✅
3. **Kept the enhanced version** with filtering capabilities ✅
4. **Validated no syntax errors** ✅
...
1. **Enhanced Instructions** - Added CI/CD prevention guidelines
2. **Pre-Deployment Checklist** - Mandatory merge conflict checks
3. **Automated Validation** - Syntax checking before deployment
4. **Documentation** - Clear issue tracking and resolution process

### **Key Lessons Learned:**
- **Always resolve Git merge conflicts** before pushing to production
- **Check for duplicate function/route definitions** in Flask applications
- **Test locally** after resolving any merge conflicts
- **Implement pre-deployment validation** to catch these issues early
- **Document CI/CD breaking issues** for future reference

**Status**: ✅ **RESOLVED** - Application successfully deployed to production