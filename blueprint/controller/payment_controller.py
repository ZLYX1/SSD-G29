...
class PaymentController:
	...
	def generate_payment_token(user_id, booking_id):
		...
		# expires_at = datetime.utcnow() + timedelta(seconds=PaymentController.TOKEN_EXPIRY_SECONDS).isoformat()
		expires_at = (datetime.utcnow() + timedelta(seconds=PaymentController.TOKEN_EXPIRY_SECONDS)).isoformat()
	...
	@staticmethod
	def validate_payment_token(user_id):
		token_data = session.get('payment_token')
		if not token_data:
			return None
		...
		if token_data['user_id'] != user_id or token_data['used'] or datetime.utcnow() > expires_at:
			return None
		...
		# return token_data  # contains token, booking_id
