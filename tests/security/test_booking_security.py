...
from blueprint.models import User, Booking, Message  # Ensure Message is imported
...
def escort_session():
    ...
    with flask_app.test_client() as client, flask_app.app_context():
        ...
        if user:
            # Delete related messages and bookings before deleting the user
            Message.query.filter_by(sender_id=user.id).delete()
            Booking.query.filter_by(seeker_id=user.id).delete()  # In case escort booked as seeker
            db.session.delete(user)
            db.session.commit()

        # Create fresh test escort
        new_user = User(
